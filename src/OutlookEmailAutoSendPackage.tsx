...
function readRecipients(requireFile = true): Recipient[] {
...
if (-not (Test-Path $manifestPath)) {
  Write-Host 'ERRORE: manifest.json non trovato.' -ForegroundColor Red
  exit 1
}
...
Write-Host ''
Write-Host 'Outlook usera la firma predefinita configurata sul PC, compreso il logo.' -ForegroundColor Yellow
Write-Host 'Ogni messaggio verra aperto per un istante per permettere a Outlook di inserire la firma.' -ForegroundColor Yellow
Write-Host ''
$confirm = Read-Host 'Per procedere digita INVIA'
if ($confirm -cne 'INVIA') {
...
try {
  ...
  Write-Host 'Impossibile avviare Outlook classico. Verifica che sia installato e configurato.' -ForegroundColor Red
  Write-Host $_.Exception.Message -ForegroundColor Red
  exit 1
}

$bodyText = [string]$manifest.body
$bodyHtml = [System.Net.WebUtility]::HtmlEncode($bodyText)
$bodyHtml = $bodyHtml -replace "`r`n", '<br>'
$bodyHtml = $bodyHtml -replace "`n", '<br>'
$bodyHtml = $bodyHtml -replace "`r", '<br>'
$sent = 0
...
foreach ($item in $items) {
  try {
    ...
    # Display forza Outlook a inserire la firma predefinita, incluse eventuali immagini/logo.
    $mail.Display($false)
    ...
    foreach ($rel in @($item.attachments)) {
      ...
      if (-not (Test-Path -LiteralPath $path)) {
        throw "Allegato non trovato: $path"
      }
...
export default function OutlookEmailAutoSendPackage() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (commonMode) {
          ...
          for (let i = 0; i < commonFiles.length; i += 1) {
            ...
            zip.file(rel, file);
            commonPaths.push(rel);
          }
          recipients.forEach((row) => {
            manifestRecipients.push({ agency: row.agency, email: row.email, attachments: commonPaths });
          });
        } else {
        ...
        zip.file(
          "manifest.json",
          JSON.stringify({ subject, body, recipients: manifestRecipients }, null, 2)
        );
        ...
        zip.file(
          ...
          [
            ...
            "",
            "1) Estrai completamente questo ZIP in una cartella.",
            "2) Consigliato: fai doppio clic su TEST_1_EMAIL.bat e verifica la prima email.",
            "3) Se il test e corretto, fai doppio clic su INVIA_TUTTE.bat.",
            "4) Nella finestra nera digita INVIA quando richiesto.",
            "",
            "La procedura usa Outlook classico installato sul PC e la firma predefinita di Outlook, compreso il logo.",
            "Per inserire la firma, le finestre dei messaggi possono apparire per un istante durante l'invio.",
            "A seconda delle policy aziendali Outlook puo mostrare un avviso di sicurezza per l'invio programmatico.",
            "La webapp non accede alla casella Outlook e non richiede autorizzazioni Microsoft Entra.",
          ].join("\r\n")
        );
    ...
    const ensureButton = () => {
      ...
      if (!button) {
        ...
        button.title = "Scarica un pacchetto che invia tutte le email tramite Outlook classico usando la tua firma predefinita";
