...
const INITIAL_ENERGY_OFFERS: EnergyOffer[] = [
  {
    ...
    canone: 16,
    spread: 0.03,
    maggiorazioneCapacityMarket: 0.01,
  },
  {
    nome: 'IMPRESAUNICA',
    canone: 15,
    spread: 0.02,
    maggiorazioneCapacityMarket: 0.008,
  },
  {
    nome: 'IMPRESASPECIAL',
    canone: 14,
    spread: 0.015,
    maggiorazioneCapacityMarket: 0.005,
  },
  {
    nome: 'SCELTA',
    canone: 16,
  ...
  },
  {
    ...
    canone: 14,
    spread: 0.011,
    maggiorazioneCapacityMarket: 0.004,
  },
  {
    nome: 'VALORE',
    canone: 15,
  ...
  },
  {
    ...
    canone: 13,
    spread: 0.015,
    maggiorazioneCapacityMarket: 0.007,
  },
  ...
  {
    nome: 'SICURADOMESTICO',
    canone: 12,
    spread: 0,
    maggiorazioneCapacityMarket: 0.011,
  },
  {
    nome: 'SICURABUSINESS',
    canone: 15,
  ...
  },
  {
    ...
    canone: 20,
    spread: 0.022,
    maggiorazioneCapacityMarket: 0.022,
  },
  {
    nome: 'CONDOMINI 10',
    canone: 25,
...
const gasBilling = ['MENSILE', 'BIMESTRALE', 'TRIMESTRALE', 'QUADRIMESTRALE'];
...
function energyPdfTipologia(tipo: string) {
  if (tipo === 'RESIDENTE' || tipo === 'RESIDENTE CANONE ESENTE')
    return 'DOMESTICO RESIDENTE';
  if (tipo === 'NON RESIDENTE') return 'DOMESTICO NON RESIDENTE';
  if (['BTA1', 'BTA2', 'BTA3', 'BTA4', 'BTA5', 'BTA6'].includes(tipo))
    return 'ALTRI USI BT';
...
function sLikeBimestrale(fatturazione: string) {
  return (
    fatturazione === 'BIMESTRALE' || fatturazione === 'MULTI POD BIMESTRALE'
  );
}
...
function calcEnergia(
  ...
  const fissoRow =
    monthlyRows.find((x) => x.mese === 'FISSO') || monthlyRows[0];

  const meseTabella1 = d.mese1 === 'FISSO' ? d.meseRifTabella1 : d.mese1;
  const meseTabella2 = d.mese2 === 'FISSO' ? d.meseRifTabella2 : d.mese2;

  const dispRow1 =
    dispCpRows.find((x) => x.mese === meseTabella1) || dispCpRows[0];
  const dispRow2 =
    dispCpRows.find((x) => x.mese === meseTabella2) || dispCpRows[0];
  ...
  const H35 = isSi(d.acciseManualiFlag)
    ? n(d.acciseManualiValore)
    : consumiTot * 0.0125;
  ...
  const risparmioFattura = isSi(d.confrontoFlag)
    ? H41 - n(d.confrontoValore)
    : 0;
  const risparmioAnnuo = isSi(d.confrontoFlag)
    ? (risparmioFattura * 12) / mesi
    : 0;
...
function calcGas(d: any, monthlyRows: MonthlyRow[], gasOffers: GasOffer[]) {
  ...
  const p1 = n(
    (monthlyRows.find((x) => x.mese === d.periodo1) || monthlyRows[0]).psv
  );
  const p2 = n(
    (monthlyRows.find((x) => x.mese === d.periodo2) || { psv: 0 }).psv
  );
  const p3 = n(
    (monthlyRows.find((x) => x.mese === d.periodo3) || { psv: 0 }).psv
  );
  const p4 = n(
    (monthlyRows.find((x) => x.mese === d.periodo4) || { psv: 0 }).psv
  );

  const consumoTotale =
    n(d.consumo1) + n(d.consumo2) + n(d.consumo3) + n(d.consumo4);
...
function Energia({
  ...
  const set = (k: string, v: string) =>
    setS((prev) => {
      ...
      if (k === 'tipo') {
        if (
          ['RESIDENTE', 'NON RESIDENTE', 'RESIDENTE CANONE ESENTE'].includes(v)
        ) {
          newState.iva = '10';
        } else if (
          [
  ...
  const consumoAnnuoEnergia =
    r.consumiTot *
    (s.fatturazione === 'MENSILE' || s.fatturazione === 'MULTI POD MENSILE'
      ? 12
      : 6);
  ...
  const printEnergyPdf = () => {
    ...
    const prezzoMedioVendita =
      r.consumiTot > 0
        ? `${(r.H22 / r.consumiTot).toFixed(6).replace('.', ',')} €/kWh`
        : '-';
    const prezzoMedioRete =
      r.consumiTot > 0
        ? `${(r.H25 / r.consumiTot).toFixed(6).replace('.', ',')} €/kWh`
        : '-';
    const prezzoMedioTotale =
      r.consumiTot > 0
        ? `${(quotaConsumiTot / r.consumiTot)
            .toFixed(6)
            .replace('.', ',')} €/kWh`
        : '-';
    ...
    const altrePartiteRows = [
      r.H38 !== 0
        ? `<tr><td>Ricalcoli/Sconti</td><td style="text-align:right">${money(
            r.H38
          )}</td></tr>`
        : '',
      r.H39 !== 0
        ? `<tr><td>Bonus sociale</td><td style="text-align:right">- ${money(
            r.H39
          )}</td></tr>`
        : '',
      r.H40 !== 0
        ? `<tr><td>Canone RAI</td><td style="text-align:right">${money(
            r.H40
          )}</td></tr>`
        : '',
    ]
    ...
    const acciseIvaRows = [
      r.H35 !== 0
        ? `<tr><td>Accise</td><td style="text-align:right">${money(
            r.H35
          )}</td></tr>`
        : '',
      r.H36 !== 0
        ? `<tr><td>IVA</td><td style="text-align:right">${money(
            r.H36
          )}</td></tr>`
        : '',
    ]
    ...
    const html = `
      <div class="page">
        ...
        <div class="box box-energy">
          ...
          <div class="grid">
            <div><div class="label">Cliente</div><div class="value">${
              s.nome || '-'
            }</div></div>
            <div><div class="label">POD</div><div class="value">${
              s.pod || '-'
            }</div></div>
            <div><div class="label">Offerta</div><div class="value">${
              s.offerta || '-'
            }</div></div>
            <div><div class="label">Tipologia</div><div class="value">${energyPdfTipologia(
              s.tipo
            )}</div></div>
            <div><div class="label">Fatturazione</div><div class="value">${
              s.fatturazione || '-'
            }</div></div>
        ...
        <div class="savings">
          <div class="savings-row">
            ...
            <strong>${money(
              r.risparmioFattura / energyMonths(s.fatturazione)
            )}</strong>
          </div>
          <div class="savings-row">
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16 }}>
          ...
          {s.offerta === 'DEDICATA' && (
            <>
              ...
              >
                ...
                {field(
                  'Quota Fissa',
                  s.dedicataQuotaFissa,
                  (v) => set('dedicataQuotaFissa', v),
                  'number'
                )}
              </div>
            </>
        ...
        >
          ...
          >
            ...
            <button
              type="button"
              onClick={() =>
                set('dispacciamentoCapacityMarket', String(r.dispCpBase))
              }
              style={{
                padding: '10px 12px',
                borderRadius: 8,
        ...
        >
          ...
          >
            {field(
              'Reattiva immessa',
              s.reattivaImmessa,
              (v) => set('reattivaImmessa', v),
              'number'
            )}
            {field(
              'Reattiva prelevata',
              s.reattivaPrelevata,
              (v) => set('reattivaPrelevata', v),
              'number'
            )}
          </div>
          <div style={{ height: 12 }} />
      ...
      >
        ...
        {previewBox(
          <>{row('Totale preventivo', money(r.H41), true, 16)}</>,
          '#fdba74'
        )}
...
function Gas({
  ...
  const r = useMemo(
    () => calcGas(s, monthlyRows, gasOffers),
    [s, monthlyRows, gasOffers]
  );
  ...
  const printGasPdf = () => {
    const periodo =
      [s.periodo1, s.periodo2, s.periodo3, s.periodo4]
        .filter(Boolean)
        .join(' / ') || '-';
    const blue = '#2563eb';
    const green = '#16a34a';
    ...
    const prezzoMedioRete =
      r.consumoTotale > 0
        ? `${(r.H23 / r.consumoTotale).toFixed(6).replace('.', ',')} €/Smc`
        : '-';
    const prezzoMedioTotale =
      r.consumoTotale > 0
      ...
      <div class="page">
        ...
        <div class="box box-gas">
          ...
          <div class="grid">
            ...
            <div><div class="label">PDR</div><div class="value">${
              s.pdr || '-'
            }</div></div>
            <div><div class="label">Offerta</div><div class="value">${
              s.offerta || '-'
            }</div></div>
            <div><div class="label">Uso</div><div class="value">${
              s.uso || '-'
            }</div></div>
            <div><div class="label">Fatturazione</div><div class="value">${
              s.fatturazione || '-'
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        >
          ...
          >
            ...
            {selectField('Uso', s.uso, (v) => set('uso', v), [
              'DOMESTICO',
              'BUSINESS',
            ])}
            {field('IVA %', s.iva, (v) => set('iva', v), 'number')}
            ...
            {selectField(
              ...
              s.offerta,
              (v) => set('offerta', v),
              gasOffers.map((x) => x.nome)
            )}
            {selectField(
              'Accisa agevolata',
              s.accisaAgevolata,
              (v) => set('accisaAgevolata', v),
              ['NO', 'SI']
            )}
            {field(
              'Valore accisa',
              s.accisaValore,
              (v) => set('accisaValore', v),
              'number'
            )}
            {field(
              'Adeguamento parametro',
              s.adeguamentoParametro,
              (v) => set('adeguamentoParametro', v),
              'number'
            )}
          </div>
          ...
          {s.offerta === 'DEDICATA' && (
            <>
              ...
              >
                ...
                {field(
                  'Quota variabile',
                  s.dedicataQuotaVariabile,
                  (v) => set('dedicataQuotaVariabile', v),
                  'number'
                )}
                {field(
                  'Quota fissa',
        ...
        >
          ...
          >
            ...
            {selectField('Mese 2', s.periodo2, (v) => set('periodo2', v), [
              '',
              ...monthlyRows.map((m) => m.mese),
            ])}
            {selectField('Mese 3', s.periodo3, (v) => set('periodo3', v), [
              '',
              ...monthlyRows.map((m) => m.mese),
            ])}
            {selectField('Mese 4', s.periodo4, (v) => set('periodo4', v), [
              '',
              ...monthlyRows.map((m) => m.mese),
            ])}
            {field(
      ...
      >
        ...
        {previewBox(
          <>
            {row(
              'Vendita materia + Adeguamento Parametro',
              money(r.H22),
              true,
              16
            )}
        ...
        {previewBox(
          <>{row('Totale preventivo', money(r.H37), true, 16)}</>,
          '#93c5fd'
        )}
...
function Listini({
  ...
  const updateEnergyOffer = (
    index: number,
    key: keyof EnergyOffer,
    value: string
  ) => {
      ...
      >
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                {['Mese', 'PUN Mono', 'PUN F1', 'PUN F2', 'PUN F3', 'PSV'].map(
                  (h) => (
                    <th
                      key={h}
                      style={{
                ...
                <tr key={row.mese}>
                  ...
                  <td style={{ padding: 8 }}>
                    <input
                      ...
                      value={row.mono}
                      onChange={(e) => updateMonthly(i, 'mono', e.target.value)}
                    />
                  </td>
                  <td style={{ padding: 8 }}>
                    <input
                  ...
                  <td style={{ padding: 8 }}>
                    <input
                      ...
                      style={{ width: 100, padding: 6 }}
                      value={row.psv}
                      onChange={(e) => updateMonthly(i, 'psv', e.target.value)}
                    />
                  </td>
                </tr>
              ))}
                  ...
                  <tr key={row.mese}>
                    ...
                    <td style={{ padding: 8 }}>
                      <input
                        ...
                        onChange={(e) =>
                          updateDispCp(i, 'dispacciamento', e.target.value)
                        }
                      />
                    </td>
                    <td style={{ padding: 8 }}>
                      <input
                        ...
                        value={row.cpMarket}
                        onChange={(e) =>
                          updateDispCp(i, 'cpMarket', e.target.value)
                        }
                      />
                    </td>
                    <td style={{ padding: 8 }}>
          ...
          >
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setGasAcciseSettings((prev) => ({
                    ...prev,
                    agevolata: n(e.target.value),
                  }))
                }
              />
            </div>
                ...
                <tr key={row.nome + i}>
                  <td style={{ padding: 8 }}>
                    <input
                      ...
                      value={row.nome}
                      onChange={(e) =>
                        updateEnergyOffer(i, 'nome', e.target.value)
                      }
                    />
                  </td>
                  <td style={{ padding: 8 }}>
                    <input
                      ...
                      value={row.spread}
                      onChange={(e) =>
                        updateEnergyOffer(i, 'spread', e.target.value)
                      }
                    />
                  </td>
                  <td style={{ padding: 8 }}>
                  ...
                  <td style={{ padding: 8 }}>
                    <input
                      ...
                      value={row.canone}
                      onChange={(e) =>
                        updateEnergyOffer(i, 'canone', e.target.value)
                      }
                    />
                  </td>
                </tr>
                ...
                <tr key={row.nome + i}>
                  <td style={{ padding: 8 }}>
                    <input
                      ...
                      value={row.nome}
                      onChange={(e) =>
                        updateGasOffer(i, 'nome', e.target.value)
                      }
                    />
                  </td>
                  <td style={{ padding: 8 }}>
                    <input
                      ...
                      value={row.spread}
                      onChange={(e) =>
                        updateGasOffer(i, 'spread', e.target.value)
                      }
                    />
                  </td>
                  <td style={{ padding: 8 }}>
                    <input
                      ...
                      onChange={(e) =>
                        updateGasOffer(i, 'quotaVariabile', e.target.value)
                      }
                    />
                  </td>
                  <td style={{ padding: 8 }}>
                    <input
                      ...
                      value={row.canone}
                      onChange={(e) =>
                        updateGasOffer(i, 'canone', e.target.value)
                      }
                    />
                  </td>
                </tr>
...
export default function App() {
  ...
  const [dispCpRows, setDispCpRows] =
    useState<DispCpRow[]>(INITIAL_DISP_CP_ROWS);
  const [energyOffers, setEnergyOffers] = useState<EnergyOffer[]>(
    INITIAL_ENERGY_OFFERS
  );
    ...
    >
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        ...
        <p style={{ color: '#475569' }}>
          Versione con Energia, Gas e Listini modificabili.
        </p>
